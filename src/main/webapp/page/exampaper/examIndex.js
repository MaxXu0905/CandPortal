...
var hasInterview = false,examTimeType=1;//1：单题计时，2：部分计时；3：混合计时
var processguide = {
	isInitGuide : false,
	nextGuide :null
};//流程引导对象
...
/*倒计时插件*/
(function($){
	var timer,originSecs=0,diffSecs=0,div,$this,$hours=$('#hours'),$minutes=$('#minutes'),$seconds=$('#seconds');
	var targetTime={};
	/*初始化计时*/
	...
	function dashChange(diffSecs){
		...
		if(	targetTime.Hours>0){
			$hours.html(targetTime.Hours+'<span style="font-size:16px">时</span>');
		}else{
			$hours.empty();
		}
		if(diffSecs==0){
		...
		if(targetTime.Hours>9 && targetTime.Minutes>9 && targetTime.Seconds>9){
				$this.addClass('countDownMid');
		}else{
				$this.removeClass('countDownMid');
		}
	}
	function timedCount()
	{
		if(!timer){
			timer=setInterval(function(){
				...
				if(diffSecs<=0){
					stopCount();
					if($.data(div, 'callback')!==undefined && typeof $.data(div, 'callback')=="function"){
						$.data(div, 'callback')();
					}
					return;
				}
...
(function($){
	...
	Exam.prototype = {
		...
		loadCamera : function(){
			...
			if(hasInterview===false && isHasVideo){
		...
		toSubscribe : function(){
			var _this = this;
			//加载摄像头
			$.subscribe('loadCamera',function(event,data){
				_this.loadCamera();
			});
			//轮询检查考试时间
			...
			$.ajax({
				  ...
				  success: function(result){
					  if(result.code=="0"){
						  ...
						  if(result.data.timerType){
							  examTimeType= getTimeType(result.data.timerType);
						  }
							if(result.data.partDatas){
								//遍历题型，正式题型
								var partdata = result.data.partDatas;
		...
		/*还原部分*/
		revertPart : function(callback){
			var _this =this;
			$.ajax({
				url:convertURL(root+"/exam/getCandidateExamInfo"),
				success : function(result){
					if(result.code=="0"){
						if(result.data){
							if(result.data.partSeq){
								//还原部分
								_this.revertObj = result.data;
							}
						}
						if(callback && typeof callback=="function"){
							callback();
...
function linkData($wrap,$model,obj,objName,data){
	if(obj[objName]){
		if($.isArray(data)){
			...
			$.observable(obj[objName]).refresh(new Array(data));
		}
	}else{
		obj[objName] = obj[objName] || {};
		if(!$.isArray(data)){
			obj[objName] = new Array(data);
		}else{
			obj[objName] = data;
		}
...
//面试上一题
function interview_prev(){
	examFormal.aotoGoPrev();
}
...
function viewWindowMin(){
		...
		if(	exam.nextPublicStep ){
			try{
				if(exam.nextPublicStep=="aotoSubmitPart"){
					 interview_close();
				}
				$.publish(exam.nextPublicStep);
				if(exam.nextPublicStep=="alertAutoSubmitPaper" || exam.nextPublicStep=="aotoSubmitPart" ){
					isGuide = false;
				}
				if(exam.nextPublicStep=="interview" && examTimeType===2){
					examFormal.resetCutDown(examFormal.getSuggesTime(),3);
				}
			 ...
			 if( examFormal.getSuggesTime()>0){
					  examFormal.resetCutDown(examFormal.getSuggesTime(),examFormal.timeOverType); 
			}else if(examFormal.getSuggesTime()==0){
				 examFormal.timeOver(examFormal.timeOverType); 
			}
				//记录开始时间
...
function getTimeType(timeType){
	if(timeType==3){
		...
		throw new Error("获取的计时模式不合法："+timeType);
	}
}